...
# Chocolatey ID (for creating a dependency)
$ChocolateyID = $(Get-IntuneWin32App -DisplayName "Chocolatey").id
...
Write-Host "Configuring the detection script to detect $ChocolateyAppName" -ForegroundColor Cyan
(Get-Content $PSScriptRoot\Detection-Sample\detect.ps1).Replace('Sample.Sample', $ChocolateyAppName) | Set-Content $PSScriptRoot\Detection-Sample\detect.ps1
...
$InstallCommandLine = "`"%systemroot%\sysnative\cmd.exe`" /c `"install.cmd`" install $ChocolateyAppName $ChocolateyInstallParams"
$UninstallCommandLine = "`"%systemroot%\sysnative\cmd.exe`" /c `"install.cmd`" uninstall $ChocolateyAppName"
...
if ($action.Contains("Add")) {
    ...
    # Configure the dependency for Chocolatey to ensure its installed prior to this app
    Write-Host "`nConfiguring dependency on Chocolatey" -ForegroundColor Cyan
    $ChocolateyDependecy = New-IntuneWin32AppDependency -ID $ChocolateyID -DependencyType AutoInstall
    Add-IntuneWin32AppDependency -ID $AppID.id -Dependency $ChocolateyDependecy
